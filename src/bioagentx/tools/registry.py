...
class ToolRegistry:
    ...
    async def call(self, name: str, tool_input: dict[str, Any]) -> tuple[ToolResult, bool]:
        if name not in self.tools:
            ...
            raise KeyError(f"Unknown BioAgentX tool: {name}")
...
def build_default_registry(cache_ttl_seconds: int = 300) -> ToolRegistry:
    return ToolRegistry(
        tools=[GeneLookupTool(), PathwayAnalysisTool(), ClinicalTrialSearchTool(), StatsAnalysisTool()],
        cache=AsyncTTLCache[ToolResult](cache_ttl_seconds),
    )
